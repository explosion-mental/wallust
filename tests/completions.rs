...
"#;
    ...
    let mut out = File::create(dir.join(&full_name))?;
    ...
    out.flush()?;
     ...
     //     let full_name = format!("{name}.1");
     //
     //     // omit `-help` subcommands (auto filled by clap)
     //     if full_name.contains(&"help") { return Ok(()); }
     //
     //     // custom: ommit debug subcommand (because it's simplicity)
     //     if full_name.contains(&"debug") { return Ok(()); }
     //
     //     let mut out = File::create(dir.join(&full_name))?;
     //
     //
     ...
     //     out.flush()?;
     //
     //     for sub in app.get_subcommands().filter(|c| !c.is_hide_set()) {
     //         print(dir, sub)?;
     //     }
     //
     //     Ok(())
