_wallust() {
    ...
    case "${cmd}" in
        wallust)
            opts="-h -V --help --version run cs theme migrate debug help"
        ...
        wallust__migrate)
            ...
            ;;
        wallust__run)
            opts="-a -b -c -C -d -f -i -k -n -p -q -s -t -T -u -w -h --alpha --backend --colorspace --config-path --config-dir --fallback-generator --ignore-sequence --check-contrast --no-cache --palette --quiet --skip-sequences --saturation --threshold --skip-templates --update-current --overwrite-cache --help <FILE>"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
