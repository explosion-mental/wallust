_wallust() {
    ...
    case "${cmd}" in
        wallust)
            opts="-i -q -s -T -u -h -V --ignore-sequence --quiet --skip-sequences --skip-templates --update-current --help --version run cs theme migrate debug help"
            ...
            ;;
        wallust__cs)
            opts="-f -i -q -s -T -u -h --format --ignore-sequence --quiet --skip-sequences --skip-templates --update-current --help <FILE>"
            ...
            ;;
        wallust__debug)
            opts="-i -q -s -T -u -h --ignore-sequence --quiet --skip-sequences --skip-templates --update-current --help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        wallust__migrate)
            ...
            ;;
        wallust__run)
            opts="-a -b -c -C -d -f -k -n -p -t -w -i -q -s -T -u -h --alpha --backend --colorspace --config-path --config-dir --fallback-generator --check-contrast --no-cache --palette --saturation --threshold --overwrite-cache --ignore-sequence --quiet --skip-sequences --skip-templates --update-current --help <FILE>"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
